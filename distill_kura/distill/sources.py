...
SOURCES: dict[str, Source] = {
    s.name: s for s in (ClaudeCodeSource(), DshSource(), EvidenceJsonlSource(), TextSource(), LnaJournalSource())
}


def source_for(path: str) -> Source | None:
    # `evidence` (*.evidence.jsonl) and `lna` (*.lna.jsonl) must be checked before
    # `claude`, which answers for ANY .jsonl (see AGENTS.md: first-match, SOURCES order irrelevant).
    for s in (SOURCES["dsh"], SOURCES["evidence"], SOURCES["lna"], SOURCES["claude"], SOURCES["text"]):
        if s.matches(path):
            return s
    return None
